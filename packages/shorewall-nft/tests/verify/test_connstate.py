...
``_ns`` (the only network side-effect) is patched to a stub that returns
...
class TestEstablishedTcp:
    def test_happy_path_returncode_zero(self):
        ...
        with patch("shorewall_nft.verify.connstate._ns", return_value=_completed(0)):
    ...
    def test_nonzero_returncode_is_failure(self):
        ...
        with patch("shorewall_nft.verify.connstate._ns", return_value=_completed(1)):
    ...
    def test_default_port_is_80(self):
        ...
        with patch("shorewall_nft.verify.connstate._ns", return_value=_completed(0)) as mock_ns:
...
class TestDropNotSyn:
    def test_dropped_output_passes(self):
        ...
        with patch("shorewall_nft.verify.connstate._ns",
    ...
    def test_exception_in_ns_is_caught(self):
        """If _ns raises, result is passed=False with detail describing the error."""
        with patch("shorewall_nft.verify.connstate._ns",
...
class TestRunSmallConntrackProbe:
    ...
    def test_value_error_in_count_treated_as_zero(self):
        ...
        with patch("shorewall_nft.verify.connstate._ns", side_effect=_bad_stdout):
            results = _run_small_conntrack_probe()
        named = {r.name: r for r in results}
        assert not named["ct:tcp_flow_tracked"].passed
