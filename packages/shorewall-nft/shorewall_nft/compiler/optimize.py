...
_COMBINABLE_FIELDS = frozenset({
    ...
    "udp dport", "udp sport",
    "iifname", "oifname",
    # ct-original variants — common in pre-DNAT match patterns,
    # generated by shorewall ORIGINAL DEST column.
    "ct original ip saddr", "ct original ip daddr",
    "ct original ip6 saddr", "ct original ip6 daddr",
    # ICMP type combines (e.g. accept echo-request + echo-reply on same iif).
    "icmp type", "icmpv6 type",
    # L4 proto combines for cross-protocol same-action rules.
    "meta l4proto",
})
...
def run_optimizations(ir: FirewallIR, level: int) -> dict[str, int]:
    ...
    if level >= 9:
        # Fixed-point iteration: each global-combine pass can unlock further
        # combines on a different field (e.g. pass 1 merges dport-sets, pass
        # 2 sees that several rules now share a dport-set and can merge their
        # daddrs). Re-running level 2 between passes catches any duplicates
        # introduced when set-merges produce identical rules.
        total_glob = 0
        total_dup = 0
        max_passes = 8  # safety cap; stable in 2-3 passes empirically
        for _ in range(max_passes):
            glob = optimize_combine_matches_global(ir)
            dup = optimize_duplicates(ir)
            total_glob += glob
            total_dup += dup
            if glob == 0 and dup == 0:
                break
        results["combine_matches_global"] = total_glob
        if total_dup:
            results["duplicates"] = results.get("duplicates", 0) + total_dup
