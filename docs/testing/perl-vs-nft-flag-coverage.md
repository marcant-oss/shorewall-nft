...
| `loopback` | (audit blind spot) | **F (parser-tolerant)** | `_parse_options` — silently accepted as a bare flag | n/a | Perl init-only annotation marking the lo iface; no nft emit semantics in either shorewall-nft or upstream Perl. shorewall-nft's existing `loopback` zone type at zones.py:65 covers the zone-side; the iface-level flag is a no-op annotation that lands in `iface.options` and is never consulted. |
| `ignore` | (audit blind spot) | **F (parser-tolerant)** | `_parse_options` — silently accepted as a bare flag | n/a | Perl numeric bitmask (`NO_UPDOWN=1`, `NO_SFILTER=2`) for init-script skip; no nft semantics. Lands in `iface.options` and is ignored. |
